...
class LLMHandler:
    ...
    async def call_llm(self, prompt: str, system_prompt: str = "", 
        ...
        if not provider:
            raise ValueError(f"找不到LLM提供商：{self.provider_id}")
        ...
        try:
            ...
            if response.role == "assistant":
                ...
                # 处理函数调用
                raise ValueError("LLM返回了工具调用响应，但当前不支持处理")
...
"""
    ...
    async def generate_markdown_document(self, metadata: Dict[str, Any], description: str) -> str:
        """生成插件Markdown文档
...
"""
    ...
    async def optimize_plugin_metadata(self, metadata: Dict[str, Any], 
...
请直接返回Python代码，包含在``python和```之间。"""
    ...
    async def review_plugin_code(self, code: str, metadata: Dict[str, Any],
        ...
        system_prompt = f"""你是一个专业的AstrBot插件代码审查员。请严格审查插件代码的安全性、质量和合规性。
...
重要：如果发现任何安全问题或违反反向提示词要求，必须将 approved 设置为 false，并给出详细的问题描述。"""
    ...
    async def fix_plugin_code(self, code: str, issues: List[str],
                            suggestions: List[str], max_retries: int = 3) -> str:
        """修复插件代码
...
6. 确保生成的内容符合反向提示词要求"""
    ...
    async def modify_config_schema(self, current_config: str, metadata: Dict[str, Any],
                             feedback: str = "") -> str:
        """修改插件配置文件
...
请直接返回修改后的JSON配置文件内容，包含在```json和```之间。"""
    ...
    async def modify_markdown_document(self, current_markdown: str, metadata: Dict[str, Any],
                                feedback: str = "") -> str:
        """修改插件Markdown文档
...
请直接返回修改后的Markdown文档内容，包含在`````和```之间。"""
    ...
    async def modify_plugin_metadata(self, current_metadata: Dict[str, Any],
                               feedback: str = "") -> Dict[str, Any]:
        """修改插件元数据
