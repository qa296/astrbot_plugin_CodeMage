...
- 🧪 **本地静态检查**: 集成 ruff / pylint / mypy 与 AstrBot 专项规则，自动审查与可选修复
- 🔐 **安全过滤**: 防止生成恶意或不适当的内容
...
| `enable_static_analysis` | 启用本地静态审查（ruff/pylint/mypy） | `true` |
| `static_analysis_strict` | 静态审查严格模式（失败即阻止继续，尝试自动修复一次） | `true` |
| `static_satisfaction_threshold` | 静态审查通过阈值（0-100） | `85` |
| `enable_astrbot_checks` | 启用 AstrBot 专项规则校验 | `true` |
